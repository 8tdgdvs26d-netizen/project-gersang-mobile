...
test('tactical pause breaks on damage, slow arrow slows, and a 5x2 ice wall blocks enemies',async()=>{
  const close=new DatabaseSync(join(dir,'test.sqlite'));close.prepare(`UPDATE battles SET status='RETREATED' WHERE status='ACTIVE'`).run();close.close();
  ...
  const wall=await post('/api/commands/battle/skill',envelope('build-ice-wall',{unitId:'guard',skillId:'ice-wall',row:2,col:15}));assert.equal(wall.status,'ACCEPTED');assert.equal(wall.data.affectedUnitIds.length,10);
