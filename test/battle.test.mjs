...
test('ultimate rune pauses the whole battle, hits every enemy, and resumes safely',async()=>{
  ...
  const pauseBody=envelope('ultimate-pause',{unitId:'hero',action:'BEGIN'}),paused=await post('/api/commands/battle/global-pause',pauseBody),pauseRetry=await post('/api/commands/battle/global-pause',pauseBody);assert.deepEqual(pauseRetry,paused);assert.equal(paused.status,'ACCEPTED');const elapsed=new DatabaseSync(join(dir,'test.sqlite'));elapsed.prepare(`UPDATE battles SET updated_at=?`).run(Date.now()-900);elapsed.prepare(`UPDATE battle_units SET last_attack_at=0 WHERE side='ENEMY'`).run();elapsed.close();let battle=await request('/api/character/char-demo/battle'),hero=battle.units.find(x=>x.id==='hero');assert.ok(battle.globalPause?.remainingMs>0);assert.equal(hero.hp,hero.maxHp);const blockedMove=await post('/api/commands/battle/move',envelope('move-during-ultimate',{unitId:'hero',row:2,col:11}));assert.equal(blockedMove.errorCode,'ERR_BATTLE_PAUSED');
  const cancelled=await post('/api/commands/battle/global-pause',envelope('ultimate-cancel',{unitId:'hero',action:'END'}));assert.equal(cancelled.status,'ACCEPTED');battle=await request('/api/character/char-demo/battle');assert.equal(battle.globalPause,null);assert.equal(battle.units.find(x=>x.id==='hero').skills.find(x=>x.id==='heaven-rune').used,false);const restarted=await post('/api/commands/battle/global-pause',envelope('ultimate-restart',{unitId:'hero',action:'BEGIN'}));assert.equal(restarted.status,'ACCEPTED');
  const cast=await post('/api/commands/battle/skill',envelope('ultimate-cast',{unitId:'hero',skillId:'heaven-rune',targetId:'hero',rune:'O'}));assert.equal(cast.status,'ACCEPTED');assert.equal(cast.data.hits.length,3);assert.equal(cast.data.damage,105);battle=await request('/api/character/char-demo/battle');assert.equal(battle.globalPause,null);assert.ok(battle.units.filter(x=>x.side==='ENEMY').every(x=>x.hp===30));const ultimate=battle.units.find(x=>x.id==='hero').skills.find(x=>x.id==='heaven-rune');assert.equal(ultimate.used,true);const second=await post('/api/commands/battle/skill',envelope('ultimate-second',{unitId:'hero',skillId:'heaven-rune',targetId:'hero',rune:'O'}));assert.equal(second.errorCode,'ERR_ULTIMATE_USED');const pauseAgain=await post('/api/commands/battle/global-pause',envelope('ultimate-pause-again',{unitId:'hero',action:'BEGIN'}));assert.equal(pauseAgain.errorCode,'ERR_ULTIMATE_USED');
});
