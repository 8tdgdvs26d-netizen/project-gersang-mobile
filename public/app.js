const S={sessionId:'',snap:null,cities:[],encounters:[],roster:[],deploymentUnitIds:null,market:[],storage:[],tx:[],battle:null,selectedUnitIds:[],focusTargetId:null,armedSkill:null,battleScrollLeft:0,battlePanDragging:false,battlePanFrame:0,hitUnitIds:[],skillEffects:[],battleLog:[],tab:'market',modal:null};
...
async function refresh(){
  ...
  const [market,storage,tx,battle,roster]=await Promise.all([req(`/api/cities/${S.snap.cityId}/market`),req(`/api/character/char-demo/storage/${S.snap.cityId}`),req('/api/character/char-demo/transactions'),req('/api/character/char-demo/battle'),req('/api/character/char-demo/roster')]);
  S.market=market;S.storage=storage;S.tx=tx;S.roster=roster;if(S.deploymentUnitIds===null)S.deploymentUnitIds=roster.map(x=>x.id);setBattle(battle);if(battle?.status==='ACTIVE')S.tab='battle';
  render();
}
...
function render(){
  ...
  document.querySelector('#app').innerHTML=`<div class="shell ${activeBattle?'combat-shell':''}"><section class="card top"><div><div class="small">Combat Prototype v0.15.1</div><div class="city">${cityName(s.cityId)}</div><div class="small">${s.state}</div></div><div class="money">💰 ${s.walletGold}</div></section>${tabs}${view()}</div>`;
...
function battleView(){
  const b=S.battle;
  if(!b||b.status!=='ACTIVE'){const label={VICTORY:'勝利',DEFEAT:'戰敗',RETREATED:'已撤退'}[b?.status]||b?.status||'',xp=b?.reward?.xpRewards?.map(x=>`<div class="xp-row"><span>${x.name} <small>Lv.${x.level}</small></span><b>+${x.xp} EXP</b><small>總 EXP ${x.totalXp}</small></div>`).join('')||'',team=S.deploymentUnitIds||[],role={MELEE:'近戰',RANGED:'遠攻'},roster=S.roster.map(x=>{const picked=team.includes(x.id),required=x.id==='hero',skills=x.skills.map(s=>s.name).join('、')||'未有主動技能';return `<button class="roster-card ${picked?'picked':''} ${required?'required':''}" data-deploy-unit="${x.id}" aria-pressed="${picked}"><span class="roster-check">${required?'◆':picked?'✓':'＋'}</span><b>${x.name} · Lv.${x.level}${required?' · 必須出戰':''}</b><small>${role[x.role]}｜EXP ${x.xpIntoLevel}/100</small><span>HP ${x.maxHp} · 攻 ${x.attack} · 射程 ${x.attackRange}</span><small>技能：${skills}</small></button>`}).join(''),encounters=S.encounters.map(x=>`<article class="encounter-card ${x.difficulty==='精英'?'elite':''}"><div><b>${x.name}</b><span>${x.difficulty} · 建議 Lv.${x.recommendedLevel}</span></div><div class="encounter-meta"><span>👺 ${x.enemyCount}</span><span>💰 ${x.rewardGold}</span><span>EXP ${x.rewardXp}/人</span></div><button class="btn ${x.difficulty==='精英'?'danger':''}" data-start-encounter="${x.id}" ${team.includes('hero')?'':'disabled'}>挑戰</button></article>`).join('');return `<section class="card battle-card result-card"><b>隊伍編成與關卡</b><p class="small">主角必須出戰；另外自由選擇最多兩名隊友。</p>${b?`<div class="last-result"><span>${b.encounter?.name||'戰鬥'}</span><div class="result ${b.status.toLowerCase()}">${label}</div>${b.reward?`<div class="battle-reward"><span>戰利金</span><b>💰 +${b.reward.gold}</b><small>已存入錢包及交易紀錄</small>${xp?`<div class="xp-list">${xp}</div>`:''}</div>`:''}</div>`:''}<div class="roster-head"><b>出戰隊伍</b><span>${team.length}/3</span></div><div class="roster-list">${roster}</div>${team.includes('hero')?'':'<div class="team-warning">主角必須出戰</div>'}<div class="encounter-list">${encounters}</div></section>`}
...
function toggleDeploymentUnit(unitId){const team=S.deploymentUnitIds||[];if(unitId==='hero')return toast('主角必須出戰');S.deploymentUnitIds=team.includes(unitId)?team.filter(id=>id!==unitId):team.length<3?[...team,unitId]:team;if(team.length===3&&!team.includes(unitId))toast('Prototype 隊伍上限為 3 人');render()}
async function startBattle(encounterId='bandit-patrol'){const team=S.deploymentUnitIds||[];if(!team.length)return toast('請先選擇出戰角色');const r=await command('/api/commands/battle/start',{encounterId,unitIds:team});if(r.status==='REJECTED')return toast(r.errorCode);S.selectedUnitIds=[team[0]];S.focusTargetId=null;S.armedSkill=null;S.battleScrollLeft=0;S.battleLog=[];toast(`${S.encounters.find(x=>x.id===encounterId)?.name||'戰鬥'}開始 · ${team.length} 人出戰`);await refresh()}
