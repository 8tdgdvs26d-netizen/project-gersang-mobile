const S={sessionId:'',snap:null,cities:[],encounters:[],market:[],storage:[],tx:[],battle:null,selectedUnitIds:[],focusTargetId:null,armedSkill:null,battleScrollLeft:0,battlePanDragging:false,battlePanFrame:0,hitUnitIds:[],skillEffects:[],battleLog:[],tab:'market',modal:null};
...
function render(){
  ...
  document.querySelector('#app').innerHTML=`<div class="shell ${activeBattle?'combat-shell':''}"><section class="card top"><div><div class="small">Combat Prototype v0.13.0</div><div class="city">${cityName(s.cityId)}</div><div class="small">${s.state}</div></div><div class="money">💰 ${s.walletGold}</div></section>${tabs}${view()}</div>`;
...
function battleView(){
  const b=S.battle;
  if(!b||b.status!=='ACTIVE'){const label={VICTORY:'勝利',DEFEAT:'戰敗',RETREATED:'已撤退'}[b?.status]||b?.status||'',xp=b?.reward?.xpRewards?.map(x=>`<div class="xp-row"><span>${x.name} <small>Lv.${x.level}</small></span><b>+${x.xp} EXP</b><small>總 EXP ${x.totalXp}</small></div>`).join('')||'',encounters=S.encounters.map(x=>`<article class="encounter-card ${x.difficulty==='精英'?'elite':''}"><div><b>${x.name}</b><span>${x.difficulty} · 建議 Lv.${x.recommendedLevel}</span></div><div class="encounter-meta"><span>👺 ${x.enemyCount}</span><span>💰 ${x.rewardGold}</span><span>EXP ${x.rewardXp}/人</span></div><button class="btn ${x.difficulty==='精英'?'danger':''}" data-start-encounter="${x.id}">挑戰</button></article>`).join('');return `<section class="card battle-card result-card"><b>選擇戰鬥關卡</b><p class="small">所有關卡自由挑戰；難度與獎勵由 Server 設定。</p>${b?`<div class="last-result"><span>${b.encounter?.name||'戰鬥'}</span><div class="result ${b.status.toLowerCase()}">${label}</div>${b.reward?`<div class="battle-reward"><span>戰利金</span><b>💰 +${b.reward.gold}</b><small>已存入錢包及交易紀錄</small>${xp?`<div class="xp-list">${xp}</div>`:''}</div>`:''}</div>`:''}<div class="encounter-list">${encounters}</div></section>`}
...
async function startBattle(encounterId='bandit-patrol'){const r=await command('/api/commands/battle/start',{encounterId});if(r.status==='REJECTED')return toast(r.errorCode);S.selectedUnitIds=['hero'];S.focusTargetId=null;S.armedSkill=null;S.battleScrollLeft=0;S.battleLog=[];toast(`${S.encounters.find(x=>x.id===encounterId)?.name||'戰鬥'}開始`);await refresh()}
