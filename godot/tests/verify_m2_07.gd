...
func _verify_persistence() -> void:
	...
	_check(int(upgraded.get("version", 0)) == SaveStore.VERSION and int(upgraded["market"]["A"]["test_good_02"]["current_stock"]) == 101 and int(upgraded["money"]) == 7777 + 171, "The next trade must write the current save with the market")
