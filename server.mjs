...
function battleSkill(env){const e=check(env);if(e)return e;const b=battleSnapshot();if(!b||b.status!=='ACTIVE')return{status:'REJECTED',errorCode:'ERR_NO_ACTIVE_BATTLE'};return idem(env.idempotencyKey,env.payload,()=>{const p=env.payload,unit=b.units.find(x=>x.id===p.unitId&&x.side==='PLAYER'&&x.alive),skill=(skillDefinitions[p.unitId]??[]).find(x=>x.id===p.skillId);if(!unit||!skill)return{status:'REJECTED',errorCode:'ERR_INVALID_SKILL_TARGET'};const target=skill.targetType==='SELF'?unit:b.units.find(x=>x.id===p.targetId&&x.side==='ENEMY'&&x.alive);if(!target)return{status:'REJECTED',errorCode:'ERR_INVALID_SKILL_TARGET'};if(skill.targetType==='ENEMY'&&Math.max(Math.abs(unit.row-target.row),Math.abs(unit.col-target.col))>skill.range)return{status:'REJECTED',errorCode:'ERR_SKILL_OUT_OF_RANGE'};const now=Date.now(),readyAt=db.prepare(`SELECT ready_at FROM battle_skill_cooldowns WHERE battle_id=? AND unit_id=? AND skill_id=?`).get(b.id,unit.id,skill.id)?.ready_at??0;if(now<readyAt)return{status:'REJECTED',errorCode:'ERR_SKILL_COOLDOWN',data:{readyInMs:readyAt-now}};let damage=0,targetHp=target.hp,killed=false;if(skill.targetType==='SELF'){db.prepare(`INSERT INTO battle_status_effects VALUES(?,?,?,?,?) ON CONFLICT(battle_id,unit_id,effect_id) DO UPDATE SET value=excluded.value,expires_at=excluded.expires_at`).run(b.id,unit.id,skill.id,skill.reduction,now+skill.durationMs)}else{const before=target.hp,after=Math.max(0,before-skill.damage);db.prepare(`UPDATE battle_units SET hp=?,alive=? WHERE battle_id=? AND id=?`).run(after,after>0?1:0,b.id,target.id);db.prepare(`UPDATE battle_units SET target_id=? WHERE battle_id=? AND id=?`).run(target.id,b.id,unit.id);damage=before-after;targetHp=after;killed=after===0;resolveBattleStatus(b.id,now)}db.prepare(`INSERT INTO battle_skill_cooldowns VALUES(?,?,?,?) ON CONFLICT(battle_id,unit_id,skill_id) DO UPDATE SET ready_at=excluded.ready_at`).run(b.id,unit.id,skill.id,now+skill.cooldownMs);return{status:'ACCEPTED',data:{unitId:unit.id,unitName:unit.name,targetId:target.id,targetPosition:{row:target.row,col:target.col},skillId:skill.id,skillName:skill.name,effectId:skill.targetType==='SELF'?skill.id:null,effectDurationMs:skill.durationMs??0,damage,targetHp,killed,readyAt:now+skill.cooldownMs}}})}
...
async function api(req,res){
  ...
  if(req.method==='GET'&&u.pathname==='/api/health')return reply(res,200,{ok:true,version:'0.8.1',phase:'P2 Defensive Status Skill'});
