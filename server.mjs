...
const skillDefinitions={hero:[{id:'heavy-strike',name:'重擊',damage:32,range:2,cooldownMs:4000,type:'NORMAL'}],archer:[{id:'heartseeker-arrow',name:'穿心箭',damage:24,range:7,cooldownMs:5000,type:'NORMAL'}]};
...
function battleSnapshot(){
  ...
  const fresh=db.prepare(`SELECT * FROM battles WHERE id=?`).get(battle.id),units=battleRows(battle.id),now=Date.now();
  return{id:fresh.id,status:fresh.status,rows:5,columns:60,units:units.map(x=>({id:x.id,side:x.side,name:x.name,role:x.role,row:x.row_no,col:x.col_no,destination:x.dest_row==null?null:{row:x.dest_row,col:x.dest_col},hp:x.hp,maxHp:x.max_hp,attack:x.attack_power,attackRange:x.attack_range,targetId:x.target_id,alive:!!x.alive,skills:(skillDefinitions[x.id]??[]).map(s=>{const readyAt=db.prepare(`SELECT ready_at FROM battle_skill_cooldowns WHERE battle_id=? AND unit_id=? AND skill_id=?`).get(fresh.id,x.id,s.id)?.ready_at??0;return{...s,readyAt,readyInMs:Math.max(0,readyAt-now)}})}))};
}
...
function battleSkill(env){const e=check(env);if(e)return e;const b=battleSnapshot();if(!b||b.status!=='ACTIVE')return{status:'REJECTED',errorCode:'ERR_NO_ACTIVE_BATTLE'};return idem(env.idempotencyKey,env.payload,()=>{const p=env.payload,unit=b.units.find(x=>x.id===p.unitId&&x.side==='PLAYER'&&x.alive),target=b.units.find(x=>x.id===p.targetId&&x.side==='ENEMY'&&x.alive),skill=(skillDefinitions[p.unitId]??[]).find(x=>x.id===p.skillId);if(!unit||!target||!skill)return{status:'REJECTED',errorCode:'ERR_INVALID_SKILL_TARGET'};if(Math.max(Math.abs(unit.row-target.row),Math.abs(unit.col-target.col))>skill.range)return{status:'REJECTED',errorCode:'ERR_SKILL_OUT_OF_RANGE'};const now=Date.now(),readyAt=db.prepare(`SELECT ready_at FROM battle_skill_cooldowns WHERE battle_id=? AND unit_id=? AND skill_id=?`).get(b.id,unit.id,skill.id)?.ready_at??0;if(now<readyAt)return{status:'REJECTED',errorCode:'ERR_SKILL_COOLDOWN',data:{readyInMs:readyAt-now}};const before=target.hp,after=Math.max(0,before-skill.damage);db.prepare(`UPDATE battle_units SET hp=?,alive=? WHERE battle_id=? AND id=?`).run(after,after>0?1:0,b.id,target.id);db.prepare(`UPDATE battle_units SET target_id=? WHERE battle_id=? AND id=?`).run(target.id,b.id,unit.id);db.prepare(`INSERT INTO battle_skill_cooldowns VALUES(?,?,?,?) ON CONFLICT(battle_id,unit_id,skill_id) DO UPDATE SET ready_at=excluded.ready_at`).run(b.id,unit.id,skill.id,now+skill.cooldownMs);resolveBattleStatus(b.id,now);return{status:'ACCEPTED',data:{unitId:unit.id,unitName:unit.name,targetId:target.id,targetPosition:{row:target.row,col:target.col},skillId:skill.id,skillName:skill.name,damage:before-after,targetHp:after,killed:after===0,readyAt:now+skill.cooldownMs}}})}
...
async function api(req,res){
  ...
  if(req.method==='GET'&&u.pathname==='/api/health')return reply(res,200,{ok:true,version:'0.8.0',phase:'P2 Multi-unit Skills'});
