...
function snapshot(){
  ...
  return {accountId:c.account_id,characterId:c.id,cityId:c.city_id,state:c.state,walletGold:c.wallet,cargo:{capacityUnits:c.cargo_capacity,usedUnits:used,stacks:stacks.map(s=>({goodTypeId:s.good_id,quantity:s.quantity,cargoUnitsPerItem:s.cargo_units}))},activeTravel:t?{travelId:'travel-demo',fromCityId:t.from_city,toCityId:t.to_city,routeEdgeIds:JSON.parse(t.route_json),startedAt:new Date(t.started_at).toISOString(),estimatedArrivalAt:new Date(t.eta).toISOString(),status:t.status}:null};
}
...
function idem(key,payload,fn){const ph=h(payload),old=db.prepare(`SELECT payload_hash,result_json FROM idempo WHERE key=?`).get(key);if(old){if(old.payload_hash!==ph)return{status:'REJECTED',errorCode:'ERR_COMMAND_CONFLICT'};return JSON.parse(old.result_json)}const r=fn();db.prepare(`INSERT INTO idempo VALUES(?,?,?)`).run(key,ph,JSON.stringify(r));return r}
...
function moveStorage(env){const e=check(env);if(e)return e;return idem(env.idempotencyKey,env.payload,()=>{const s=snapshot(),p=env.payload;if(s.state!=='IN_CITY'||s.cityId!==p.cityId)return{status:'REJECTED',errorCode:'ERR_PHYSICAL_PRESENCE_REQUIRED'};if(p.quantity<=0)return{status:'REJECTED',errorCode:'ERR_INVALID_QUANTITY'};db.exec('BEGIN IMMEDIATE');try{const cur=db.prepare(`SELECT quantity FROM storage WHERE character_id='char-demo' AND city_id=? AND good_id=?`).get(s.cityId,p.goodTypeId)?.quantity??0;if(p.direction==='CARGO_TO_STORAGE'){changeCargo(p.goodTypeId,-p.quantity);db.prepare(`INSERT INTO storage VALUES('char-demo',?,?,?) ON CONFLICT(character_id,city_id,good_id) DO UPDATE SET quantity=quantity+excluded.quantity`).run(s.cityId,p.goodTypeId,p.quantity)}else{if(cur<p.quantity)throw new Error('ERR_INSUFFICIENT_STORAGE');db.prepare(`UPDATE storage SET quantity=quantity-? WHERE character_id='char-demo' AND city_id=? AND good_id=?`).run(p.quantity,s.cityId,p.goodTypeId);changeCargo(p.goodTypeId,p.quantity)}db.exec('COMMIT');return{status:'ACCEPTED',data:{quantity:p.quantity}}}catch(err){db.exec('ROLLBACK');return{status:'REJECTED',errorCode:err.message}}})}

async function api(req,res){
  const u=new URL(req.url,'http://localhost');
  if(req.method==='GET'&&u.pathname==='/api/health')return reply(res,200,{ok:true,version:'0.4',phase:'First Playable'});
...
server.listen(PORT,'0.0.0.0',()=>console.log(`FIRST_PLAYABLE_READY http://0.0.0.0:${PORT}`));
