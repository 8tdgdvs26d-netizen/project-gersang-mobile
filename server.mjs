...
function startBattle(env){const e=check(env);if(e)return e;return idem(env.idempotencyKey,env.payload,()=>{const s=snapshot();if(s.state!=='IN_CITY')return{status:'REJECTED',errorCode:'ERR_INVALID_STATE'};const old=db.prepare(`SELECT id FROM battles WHERE character_id='char-demo' AND status='ACTIVE'`).get();if(old)return{status:'REJECTED',errorCode:'ERR_BATTLE_ALREADY_ACTIVE'};const encounter=encounterDefinitions[env.payload.encounterId??'bandit-patrol'];if(!encounter)return{status:'REJECTED',errorCode:'ERR_ENCOUNTER_NOT_FOUND'};const requested=env.payload.unitIds===undefined?playerTemplates.map(x=>x.id):[...new Set(Array.isArray(env.payload.unitIds)?env.payload.unitIds:[])],available=new Set(playerTemplates.map(x=>x.id));if(!requested.length)return{status:'REJECTED',errorCode:'ERR_TEAM_REQUIRED'};if(requested.length>3||requested.some(id=>!available.has(id)))return{status:'REJECTED',errorCode:'ERR_INVALID_TEAM'};if(!requested.includes('hero'))return{status:'REJECTED',errorCode:'ERR_HERO_REQUIRED'};const id=randomUUID(),now=Date.now();db.prepare(`INSERT INTO battles VALUES(?,'char-demo','ACTIVE',?,?)`).run(id,now,now);db.prepare(`INSERT INTO battle_meta VALUES(?,?,?,?)`).run(id,encounter.id,encounter.rewardGold,encounter.rewardXp);const add=db.prepare(`INSERT INTO battle_units VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`),levels=new Map(db.prepare(`SELECT unit_id,level FROM roster_units WHERE character_id='char-demo'`).all().map(x=>[x.unit_id,x.level]));for(const p of playerTemplates.filter(x=>requested.includes(x.id))){const level=levels.get(p.id)??1,hp=p.baseHp+(level-1)*p.hpPerLevel,attack=p.baseAttack+(level-1)*p.attackPerLevel;add.run(id,p.id,'PLAYER',p.name,p.role,p.row,p.col,null,null,hp,hp,attack,p.range,p.interval,now,null,1)}for(const [uid,name,role,row,col,hp,attack,range,interval] of encounter.enemies)add.run(id,uid,'ENEMY',name,role,row,col,null,null,hp,hp,attack,range,interval,now,null,1);return{status:'ACCEPTED',data:{battleId:id,encounterId:encounter.id,unitIds:requested}}})}
