...
const playerTemplates=[{id:'hero',name:'主角',role:'MELEE',row:2,col:2,baseHp:120,hpPerLevel:12,baseAttack:18,attackPerLevel:2,range:2,interval:900},{id:'archer',name:'弓手',role:'RANGED',row:1,col:1,baseHp:80,hpPerLevel:8,baseAttack:13,attackPerLevel:2,range:5,interval:750},{id:'guard',name:'護衛',role:'MELEE',row:3,col:1,baseHp:105,hpPerLevel:15,baseAttack:15,attackPerLevel:1,range:2,interval:1000}];
...
function rosterSnapshot(){return playerTemplates.map(p=>{const r=db.prepare(`SELECT level,xp FROM roster_units WHERE character_id='char-demo' AND unit_id=?`).get(p.id)??{level:1,xp:0};return{id:p.id,name:p.name,role:p.role,level:r.level,xp:r.xp,xpIntoLevel:r.xp%100,xpToNext:100-r.xp%100,maxHp:p.baseHp+(r.level-1)*p.hpPerLevel,attack:p.baseAttack+(r.level-1)*p.attackPerLevel,attackRange:p.range,skills:(skillDefinitions[p.id]??[]).map(({id,name,type,targetType,description,damage,range,cooldownMs})=>({id,name,type,targetType,description,damage,range,cooldownMs}))}})}
function grantBattleReward(id,now=Date.now()){const battle=db.prepare(`SELECT b.character_id,COALESCE(m.reward_gold,100) reward_gold,COALESCE(m.reward_xp,50) reward_xp FROM battles b LEFT JOIN battle_meta m ON m.battle_id=b.id WHERE b.id=? AND b.status='VICTORY'`).get(id);if(!battle)return null;db.prepare(`INSERT OR IGNORE INTO battle_rewards VALUES(?,?,?,?)`).run(id,battle.character_id,battle.reward_gold,now);db.prepare(`INSERT OR IGNORE INTO battle_xp_rewards SELECT battle_id,id,? FROM battle_units WHERE battle_id=? AND side='PLAYER' AND alive=1`).run(battle.reward_xp,id);return db.prepare(`SELECT gold,granted_at FROM battle_rewards WHERE battle_id=?`).get(id)}
...
function startBattle(env){const e=check(env);if(e)return e;return idem(env.idempotencyKey,env.payload,()=>{const s=snapshot();if(s.state!=='IN_CITY')return{status:'REJECTED',errorCode:'ERR_INVALID_STATE'};const old=db.prepare(`SELECT id FROM battles WHERE character_id='char-demo' AND status='ACTIVE'`).get();if(old)return{status:'REJECTED',errorCode:'ERR_BATTLE_ALREADY_ACTIVE'};const encounter=encounterDefinitions[env.payload.encounterId??'bandit-patrol'];if(!encounter)return{status:'REJECTED',errorCode:'ERR_ENCOUNTER_NOT_FOUND'};const requested=env.payload.unitIds===undefined?playerTemplates.map(x=>x.id):[...new Set(Array.isArray(env.payload.unitIds)?env.payload.unitIds:[])],available=new Set(playerTemplates.map(x=>x.id));if(!requested.length)return{status:'REJECTED',errorCode:'ERR_TEAM_REQUIRED'};if(requested.length>3||requested.some(id=>!available.has(id)))return{status:'REJECTED',errorCode:'ERR_INVALID_TEAM'};if(!requested.includes('hero'))return{status:'REJECTED',errorCode:'ERR_HERO_REQUIRED'};const supplied=env.payload.deployments,deployments=supplied===undefined?playerTemplates.filter(x=>requested.includes(x.id)).map(x=>({unitId:x.id,row:x.row,col:x.col})):supplied;if(!Array.isArray(deployments)||deployments.length!==requested.length||new Set(deployments.map(x=>x.unitId)).size!==requested.length||deployments.some(x=>!requested.includes(x.unitId)||!Number.isInteger(x.row)||!Number.isInteger(x.col)||x.row<0||x.row>=5||x.col<0||x.col>=10)||new Set(deployments.map(x=>`${x.row}:${x.col}`)).size!==deployments.length)return{status:'REJECTED',errorCode:'ERR_INVALID_DEPLOYMENT'};const positions=new Map(deployments.map(x=>[x.unitId,x])),id=randomUUID(),now=Date.now();db.prepare(`INSERT INTO battles VALUES(?,'char-demo','ACTIVE',?,?)`).run(id,now,now);db.prepare(`INSERT INTO battle_meta VALUES(?,?,?,?)`).run(id,encounter.id,encounter.rewardGold,encounter.rewardXp);const add=db.prepare(`INSERT INTO battle_units VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`),levels=new Map(db.prepare(`SELECT unit_id,level FROM roster_units WHERE character_id='char-demo'`).all().map(x=>[x.unit_id,x.level]));for(const p of playerTemplates.filter(x=>requested.includes(x.id))){const level=levels.get(p.id)??1,hp=p.baseHp+(level-1)*p.hpPerLevel,attack=p.baseAttack+(level-1)*p.attackPerLevel,pos=positions.get(p.id);add.run(id,p.id,'PLAYER',p.name,p.role,pos.row,pos.col,null,null,hp,hp,attack,p.range,p.interval,now,null,1)}for(const [uid,name,role,row,col,hp,attack,range,interval] of encounter.enemies)add.run(id,uid,'ENEMY',name,role,row,col,null,null,hp,hp,attack,range,interval,now,null,1);return{status:'ACCEPTED',data:{battleId:id,encounterId:encounter.id,unitIds:requested,deployments}}})}
...
async function api(req,res){
  ...
  if(req.method==='GET'&&u.pathname==='/api/health')return reply(res,200,{ok:true,version:'0.16.0',phase:'P2 Pre-Battle Deployment'});
